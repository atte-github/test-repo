...
# create a secret with wildcard policy
resource "buildkite_cluster_secret" "feature_secret" {
  cluster_id  = buildkite_cluster.primary.uuid
  key         = "FEATURE_API_KEY"
  value       = "feature-secret-value"
  description = "API key for feature branch testing"
  
  policy = <<-EOT
    - pipeline_slug: feature-testing
...
```shell
terraform import buildkite_cluster_secret.api_key <secret-id>
```
